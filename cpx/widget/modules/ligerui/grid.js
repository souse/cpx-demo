...
(function($) {
    ...
    $.ligerDefaults.Grid.formatters['numberbox'] = function (value, column) {
        ...
        column.editor == undefined ? precision = 2 : precision = column.editor.precision;
    ...
    $.ligerDefaults.Grid.editors['numberbox'] = {
        create: function (container, editParm) {
            ...
            var input = $("<input type='text' style='text-align:left' class='l-text-field' />");
        ...
        resize: function (input, width, height, editParm) {
            input.width(width-5).height(height);//暂时这么做
            input.focus();
        }
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' maxlength='3' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadServerData: function(param, clause, sourceType) {
            ...
            var ajaxOptions = {
                ...
                success: function(data) {
                    //code here  判断返回数据状态 && 格式化返回数据
                    //var res = data;
                    if(data.status === 0){
                        data = data.data;
                    }else{
                        console.log('ligerui Grid 获取后台数据异常，错误码：' + data.status);
                        console.log('ligerui Grid 获取后台数据异常，错误信息：' + data.msg);
                    }
        ...
        */
        ...
        enabledFrozen: function() {
            ...
            var cols = g.columns || [];
            if (g.enabledDetail() && p.frozenDetail || g.enabledCheckbox() && p.frozenCheckbox || p.frozenRownumbers && p.rownumbers) return true;
        ...
        */
        ...
        setCellEditing: function(rowdata, column, editing) {
            ...
            if (rowdata['__id'] != 0) {
                ...
                if (!prevrowobj.length || prevrowobj.length <= 0 || prevrowobj[0].id == null || prevrowobj[0].id == "") {
                    return;
                }
        ...
        _getTotalCellContent: function(column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum'))
                        totalsummaryArr.push("<div>合计：" + info.sum.toFixed(2) + "</div>");
                    if (isExist('tsum'))

...
(function($) {
    ...
    $.ligerDefaults.Tab = {
        ...
        onAfterLeaveTabItem: null //增加离开页签回调
    };
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            g.tab.links = $('<div class="l-tab-links"><ul style="left: 20px; "></ul><div class="l-tab-switch"></div></div>');
        ...
        moveToPrevTabItem: function(tabid) {
            ...
            for (var i = 0, l = tabItems.length; i < l; i++) {
                ...
                currentWidth += tabitem.outerWidth() + parseInt(tabitem.css("marginLeft")) + parseInt(tabitem.css("marginRight"));
            }
        ...
        moveToNextTabItem: function(tabid) {
            ...
            if (moveToTabItem == null) return false;
            //计算出正确的移动位置
            var left = currentWidth - (nextBtnOffset.left - prevBtnOffset.left) + parseInt(moveToTabItem.css("marginLeft")) + parseInt(moveToTabItem.css("marginRight"));
            g.tab.links.ul.animate({
                left: -1 * left
            });
            return true;
        ...
        overrideTabItem: function(targettabid, options) {
            ...
            //modified s
            var iframeloading = $("div:first", contentitem).show();
            //modified e
            ...
            $("iframe", contentitem).attr("src", url).bind('load.tab', function(){
                 iframeloading.hide();
                 if (options.callback)
                     options.callback();
            });
        ...
        selectTabItem: function(tabid) {
            ...
            if (g.trigger('beforeSelectTabItem', [tabid]) == false)
                return false;
            //增加离开页签的回调  后续会打开 ==code here==涉及到=
            //g.trigger('afterLeaveTabItem', [g.selectedTabId]);
            g.selectedTabId = tabid;
        ...
        addTabItem: function(options) {
            var g = this,
                p = this.options;
            if (g.trigger('beforeAddTabItem', [options]) == false)//[options]
                return false;
            var tabid = options.tabid;
            if (tabid == undefined) tabid = g.getNewTabid();
            var url = options.url,
            ...
            if (g.isTabItemExist(tabid)) {
                //modified s
                var curUrl = $('.l-tab-content-item[tabid='+tabid+']').find('iframe').attr('src');
                
                g.selectTabItem(tabid);
                if(curUrl != url){
                    g.overrideTabItem(tabid, options);
                    return;
                } 
